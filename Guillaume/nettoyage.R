...
library(tidyr)
library(stringr)
library(RPushbullet)#Json

### Non necessaire maintenant
...
####################Ajout des descriptions et image publisher ###########################

data<-readRDS("data_jeux.rds")
data_desc_publisher<-read_json("desc_publisher.json", simplifyVector = TRUE)
saveRDS(data_desc_publisher, "data_publisher.rds")

df<-readRDS("data_publisher.rds")
data<-left_join(data, data_desc_publisher, by="Publisher")
saveRDS(data, "data_jeux.rds")

##################Ajout des images de jeux##########################
data<-readRDS("data_jeux.rds")

data_image<-read_json("data_image.json",simplifyVector = TRUE)
data_image<-rename.variable(data_image, "titre", "Name")
data_image<-data_image[,c(1,4)]
data_image<-unique(data_image)
data_image=data_image[!duplicated(data_image),]
saveRDS(data_image, "data_jeux_image.rds")
